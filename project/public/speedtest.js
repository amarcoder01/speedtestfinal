...
Speedtest.prototype = {
  ...
  start: function() {
    ...
    try {
      // Try to load the worker with a relative path
      console.log("Attempting to load worker from: speedtest_worker.js");
      // Use the preloaded worker URL if available, otherwise fall back to direct loading
      if(typeof workerUrl !== 'undefined') {
        console.log("Using preloaded worker URL");
        this.worker = new Worker(workerUrl);
      } else {
        console.log("Falling back to direct worker loading");
        this.worker = new Worker("speedtest_worker.js?r=" + Math.random());
      }
      console.log("Worker loaded successfully");
    } catch (error) {
      console.error("Failed to initialize speed test worker: " + error);
      alert("Failed to initialize speed test. Error: " + error);
...
// Make Speedtest available globally
